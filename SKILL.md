...
- **If you can run a shell/CLI (e.g. Claude Code):** deploy via the `fe-databricks-tools:databricks-resource-deployment` skill (it handles Jobs + Lakeflow pipelines, prefers serverless, uses `databricks sync`, and UC 3-layer namespaces). Trigger it with: "use the databricks-resource-deployment skill to deploy this bundle". You own the full deploy→run→fix→redeploy loop.
