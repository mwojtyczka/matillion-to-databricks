...
- **Ask whether the source data already exists, or must be fabricated — synthetic data is a *fallback*, not the default.** List the **source/input tables the transforms read that no task in the Job produces** (read nodes with no incoming edge — the same set Step 5c would otherwise fabricate). Then decide, by an explicit predicate — *do these tables already exist in the target workspace?*
  - **Interactive session (Claude Code with a human, or Genie chat):** ask the user — "these transforms read `<tables>`; do those already exist in the target catalog/schema (and where), or should I fabricate synthetic stand-ins for a test run?" Ask this **now**, in the inventory, not at Step 5c — the answer decides whether Step 5c happens at all, and real data is what makes Step 6b gold reconciliation meaningful.
  - **Autonomous mode (unattended run):** you can't ask — default to **synthetic** (the safe fallback: a bundle that fabricates its own test data always runs), and record the assumption in the bundle README so a human can point it at real tables later.
  - **Real data exists** → point the bundle's namespace at it and **skip the synthetic setup notebook entirely** (Step 5c is not emitted); note in the README where the real sources live. **No real data** → emit the synthetic setup notebook per Step 5c.

Write down: the list of orchestration pipelines, what each one calls (transformations and nested orchestrations), the variables in play, the secrets in play (and their current source), every hardcoded value worth surfacing, any transformation not called by anything (a standalone pipeline), and **the data-source decision — real tables (and where) vs. synthetic fabrication** (this gates Step 5c).
...
- a **setup notebook** (`src/setup/00_generate_source_data.py`) the user runs **manually once** before the first run to fabricate any missing source/input tables with synthetic data for a test — kept out of the Job graph, guarded so it no-ops against real sources (see Step 5c). **Emit this only when the Step 1 data-source decision was "no real data / fabricate"** — when the real source tables already exist, skip it entirely (the bundle just reads them).
...
2. **Bundle layout** — a short tree of what was emitted (`databricks.yml`, `resources/`, `src/notebooks`, `src/dq`, and — only if synthetic data was fabricated — `src/setup` with the Step 5c setup notebook) with a one-line purpose per entry.
...
8. **Deployment** — the config values to set (`catalog`, `schema`, `warehouse_id`, host) and the exact commands **in order**, with the manual step called out as required, not optional:
   ...
   3. **Manually run** the setup notebook `src/setup/00_generate_source_data.py` once (open it in the workspace, set the `catalog`/`schema` widgets, Run All) — **required** for a test run on a fresh workspace; skip *only* if the real source tables already exist. **Omit this whole step if no synthetic notebook was emitted** (the migration targets real, existing tables) — there's nothing to run.
...
9. **Post-migration checklist** — literal checkboxes for the manual steps a human must do. Include both the **test-run** step and the **production** steps:
   - [ ] *(only if a synthetic setup notebook was emitted)* **Run `src/setup/00_generate_source_data.py` by hand** before the first `bundle run` (for a test on a fresh workspace) — it's not a Job task, so nothing runs it for you.
   - [ ] fill in real `workspace.host` per target, supply the real `warehouse_id`
   - [ ] create the secret scopes and grant READ
   - [ ] ensure the source tables the Job reads exist in the target namespace — if a synthetic notebook was emitted, production simply skips it once the **real** tables are present; if the migration already targets real tables, confirm they're there before the first run
...
**Do this step only when the Step 1 data-source decision was "no real data / fabricate."** If the real source tables already exist in the target workspace, **skip this step entirely** — don't author, run, or coverage-check a setup notebook; point the bundle at the real tables and move to Step 5d. (Synthetic data is the fallback for a fresh workspace, not a default artifact of every migration.)
...
**The deployment procedure has these ordered steps — the download and (when a synthetic setup notebook was emitted) the manual setup-notebook run are part of it, not afterthoughts:**
...
4. **Run the setup notebook** `src/setup/00_generate_source_data.py` **once, manually** (open it, set the `catalog`/`schema` widgets, Run All — or `databricks jobs submit` a one-off notebook task) — **only when one was emitted** (the "fabricate" branch of the Step 1 data-source decision). This creates the synthetic source tables the Job reads. When present it's **required for a test run on a fresh workspace** — the notebook is deliberately not a Job task, so nothing runs it automatically, and a `bundle run` without it fails at the first read (`TABLE_OR_VIEW_NOT_FOUND`). **Omit this step entirely when the migration targets real, existing source tables** (no notebook was emitted) — there's nothing to run.
5. **Run** the Job — `databricks bundle run <job>`.

Whoever performs these steps depends on where you (the agent) are running — but always present them in order; flag step 4 as required *when a setup notebook exists*, and drop it otherwise.
...
1. **Coverage-gate the setup notebook first — if one was emitted.** When the migration fabricates synthetic data, run `python3 scripts/check_setup_coverage.py <bundle-dir>` and fix any missing columns *before* the first run — this pre-empts the single most common failure class (`UNRESOLVED_COLUMN` on a missing source column), which otherwise surfaces one task-run at a time. (No setup notebook — real, existing tables — skip this; there's nothing to coverage-check.)
2. **Deploy + (run setup, if any) + run the Job** (Step 6, with the `--var`s).
